...
const getServiceLink = () => {
  const footerOrder = ['Startup Centre', "Intellectual Property", "Registration", "Certification", "Statuary Compliance"]
  servicelist = menuList?.data.filter(menu => menu.title === 'Services')[0]?.children
        ...
        <div className="container pb-2">
          <div className='primary-links-cont d-flex flex-wrap align-items-center justify-content-center justify-content-md-between  '>
            <div className='d-flex flex-wrap align-items-center justify-content-center justify-content-md-start'>
              <div className='logo-cont mt-4' data-aos="fade-up" data-aos-delay="0">
                <img src={logo} alt="Aanoor Logo" />
              </div>
              <div className='primary-link d-flex text-white pt-4 px-3'>{
                primarylist.map((list, index) => <a className='px-3' data-aos="fade-up" data-aos-delay={50 * (index + 1)} key={list.title} href={list.pageUrl}>
                  {list.title}
                </a>)}
              </div>
            </div>
            <div className='social-link-cont d-flex align-items-center pt-4'>
              <div data-aos="fade-up" data-aos-delay="250"><a href='https://www.linkedin.com/in/aanoorglobal/'><img src={linkedin} alt="linkedin link" /></a></div>
              <div data-aos="fade-up" data-aos-delay="300" className='px-3'><a href='https://www.facebook.com/aanoorgroup'><img src={facebook} alt="Facebook link" /></a></div>
              <div data-aos="fade-up" data-aos-delay="350"><a href='https://twitter.com/aanoorgroup'><img src={twitter} alt="twitter link" /></a></div>
            </div>
          </div>
          <div className='service-links-cont d-flex flex-wrap justify-content-center justify-content-lg-between text-white pt-4'>
            {servicelist.map((service, index) =>
              <div className='cat-cont' data-aos="fade-up" data-aos-delay={(index * 50) + 400} key={service.title}>
                <div className='title'>{service.title}</div>
                <ul>{
                  service.children.map(childs =>
                    <li key={childs.title}><a href={childs.pageUrl}>{childs.title}</a></li>
                  )}
                </ul>
              </div>
            )}
          ...
          <div className='copyright-cont d-flex flex-md-row flex-column flex-wrap text-white align-items-center justify-content-center justify-content-md-between '>
            <div className='copyright mb-2'>© 2010 - 2023 Aanoor Global. All Right Reserved.</div>
            <div className='mb-3'>
              Developed with <span className="love">♥</span> by <a href="https://www.praga.co.in" rel="noreferrer" target="_blank">Praga.co.in</a>
            </div>
          </div>
        </div>
